...
export const metadata = {
  ...
  description:
    "مقالات علمی خود را در دانشران منتشر کنید و با جامعه پژوهشگران ایران به اشتراک بگذارید.",
  keywords: [
    "ارسال مقاله",
    "انتشار مقاله",
    "مقالات علمی",
  ...
  openGraph: {
    ...
    locale: "fa_IR",
    siteName: "دانشران",
    url: `${baseUrl}/articles/new`,
    images: [
      {
        url: `${baseUrl}/daneshran.webp`,
        width: 1200,
        height: 630,
        alt: "ارسال مقاله جدید در دانشران",
      },
    ],
  },
  twitter: {
  ...
  alternates: {
    canonical: `${baseUrl}/articles/new`,
  },
};

export default function page() {
  return (
    <main className="mt-12 mb-6 overflow-x-hidden px-3 sm:mt-18 sm:mb-10 sm:px-4 md:px-6 lg:px-0">
      <PublishHeader />
...
function PublishHeader() {
  return (
    <header className="flex flex-col items-center justify-center gap-4 sm:gap-8">
      <div
        className="bg-brand-2 flex h-16 w-16 items-center justify-center rounded-full text-2xl text-white sm:h-18 sm:w-18 sm:text-3xl"
        aria-hidden="true"
      >
      ...
      <div className="border-brand-10 bg-brand-9 text-brand-2 flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs font-semibold sm:px-3 sm:py-1 sm:text-sm">
        <ShieldCheckIcon size={1.2} aria-hidden="true" /> انتشار امن و حرفه‌ای
      </div>
    </header>
  );
}
...
function PublishSidebar() {
  return (
    <aside className="space-y-5 lg:sticky lg:top-20 lg:h-fit lg:flex-[0.25]">
      <PublishProgress />
      <WritingTips />
      <SupportSection />
    </aside>
...
function PublishForm() {
  return (
    <article className="border-brand-7 flex-[0.75] space-y-6 rounded-xl border bg-white p-4 sm:p-5 md:p-6 lg:p-7">
      <div className="border-brand-7 border-b pb-6 sm:pb-8">
        <div className="flex flex-col items-start gap-3 sm:flex-row sm:items-center sm:justify-between">
          <h2 className="flex items-center gap-2 text-[1rem] font-semibold sm:gap-3 sm:text-[1.1rem] md:text-[1.2rem] lg:text-[1.32rem]">
            <FileTextIcon size={1} aria-hidden="true" />
            اطلاعات مقاله
          </h2>
          <Link
            href="/help"
            className="bg-brand-9 hover:bg-brand-5 text-brand-2 flex cursor-pointer items-center gap-1 rounded-full px-2 py-1 text-xs font-semibold duration-300 sm:px-3 sm:py-2 sm:text-sm"
            aria-label="راهنمای انتشار مقاله"
          >
            <InfoIcon size={1.2} aria-hidden="true" />
      ...
      <div className="mt-6 flex flex-col gap-3 sm:mt-8 sm:flex-row sm:gap-0 sm:space-x-3 md:mt-10">
        <Button size="small" aria-label="ارسال مقاله برای انتشار">
          <SendIcon size={1} aria-hidden="true" />
          <span className="text-xs sm:text-sm">ارسال مقاله</span>
        </Button>
        <Button type={2} size="small" aria-label="ذخیره پیش‌نویس مقاله">
          <SaveIcon size={1.1} aria-hidden="true" />
    ...
    <section className="border-brand-7 space-y-5 border-b border-dashed pb-5 sm:space-y-7 sm:pb-7">
      ...
      <div>
        <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
          ...
          <span className="text-brand-4 text-[0.7rem] font-medium sm:text-[0.8rem]">
            ۰ / ۱۵۰
          </span>
        </div>
        <Input
          id="article-title"
          placeholder="عنوان جذاب و دقیق برای مقاله خود انتخاب کنید..."
          aria-label="عنوان مقاله"
          aria-required="true"
        />
        <p className="text-brand-4 mt-2 flex items-center gap-1 text-[0.7rem] sm:mt-3 sm:text-[0.8rem]">
          <InfoIcon size={1.5} aria-hidden="true" />
    ...
    <section className="space-y-4 sm:space-y-5">
      ...
      <div>
        <label className="text-brand-8 flex flex-wrap items-center gap-1 text-[0.75rem] font-medium sm:space-x-1 sm:text-[0.85rem] md:text-[0.9rem]">
          ...
          <Link
            href="/rules"
            className="text-brand-2 hover:text-brand-3 cursor-pointer duration-150 hover:underline"
          >
            می‌پذیرم
          </Link>
          <span className="text-red-600" aria-hidden="true">
